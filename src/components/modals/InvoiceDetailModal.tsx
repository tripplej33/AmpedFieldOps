...
export default function InvoiceDetailModal({ invoice, open, onOpenChange }: InvoiceDetailModalProps) {
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-start justify-between">
            <div>
              <DialogTitle className="text-2xl font-bold font-mono">{invoice.invoice_number}</DialogTitle>
              <p className="text-sm text-muted-foreground mt-1">{invoice.client_name || 'Unknown Client'}</p>
            </div>
          ...
          <Card className="p-6 bg-card border-border">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm font-mono mt-1">{invoice.currency || 'USD'}</p>
              </div>
              {(invoice as any).xero_invoice_id && (
                <div>
                  <Label className="text-xs font-mono uppercase tracking-wider text-muted-foreground">Xero ID</Label>
                  <p className="text-sm font-mono mt-1">{(invoice as any).xero_invoice_id}</p>
                </div>
              )}
            </div>
          </Card>
