...
import { FileText, Calendar, ShoppingCart, CheckCircle, Clock } from 'lucide-react';
...
export default function PurchaseOrderDetailModal({ purchaseOrder, open, onOpenChange }: PurchaseOrderDetailModalProps) {
  if (!purchaseOrder) return null;

  const getStatusBadge = (status: string) => {
    const statusConfig: Record<string, { color: string; icon: React.ReactNode }> = {
      BILLED: { color: 'bg-voltage/20 text-voltage border-voltage/30', icon: <CheckCircle className="w-3 h-3" /> },
      ...
      CANCELLED: { color: 'bg-destructive/20 text-destructive border-destructive/30', icon: <FileText className="w-3 h-3" /> },
    };
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-start justify-between">
            <div>
              <DialogTitle className="text-2xl font-bold font-mono">{purchaseOrder.po_number}</DialogTitle>
              <p className="text-sm text-muted-foreground mt-1">{purchaseOrder.supplier_name || 'Unknown Supplier'}</p>
            </div>
          ...
          <Card className="p-6 bg-card border-border">
            <div className="grid grid-cols-2 gap-4">
              ...
              {purchaseOrder.delivery_date && (
                <div>
                  <Label className="text-xs font-mono uppercase tracking-wider text-muted-foreground">Delivery Date</Label>
                  <p className="text-sm font-mono mt-1">
              ...
              <div>
                ...
                <p className="text-sm mt-1">{purchaseOrder.project_name || purchaseOrder.project_code || '-'}</p>
              </div>
              <div>
                <Label className="text-xs font-mono uppercase tracking-wider text-muted-foreground">Currency</Label>
                <p className="text-sm font-mono mt-1">{purchaseOrder.currency || 'USD'}</p>
              </div>
              {(purchaseOrder as any).xero_po_id && (
                <div>
                  <Label className="text-xs font-mono uppercase tracking-wider text-muted-foreground">Xero ID</Label>
                  <p className="text-sm font-mono mt-1">{(purchaseOrder as any).xero_po_id}</p>
                </div>
              )}
            </div>
            {purchaseOrder.notes && (
            ...
            {lineItems.length === 0 ? (
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-12 gap-2 pb-2 border-b border-border text-xs font-mono uppercase tracking-wider text-muted-foreground">
                  ...
                  <div className="col-span-3 text-right">Line Amount</div>
                </div>
