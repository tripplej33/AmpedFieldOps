...
- Test RLS policies with the appropriate JWTs and role claims; document expected access for anon vs authenticated users.- **Frontend in Docker cannot access `localhost:54321` - Supabase runs on host**: Use `host.docker.internal:54321` in docker-compose.yml environment variables AND add `extra_hosts` mapping for proper host access from containers.
